...
def out_put_dictionary(patients_comment, columns_dictionary=columns_dictionary_1):
    ...
    str_output = chat_to_gpt_4o_temperature_0(prompt)
...
def create_case_dict(patients_comment, next_question):
    ...
    for i in range(len(next_question)):
        ...
        - 改行もしないでください。
        """
        str_response = chat_to_gpt_4o_temperature_0(prompt)
        # ここで str_response は 例: "0" や "カフェインのとりすぎです。" が返ってくると想定
    ...
    if user_input:
        ...
        if st.session_state.step == 1:
            ...
            st.session_state["messages"].append({
                ...
                "typed": False  # 新規メッセージはアニメーション対象
            })

            # case_dict, symptom_dictionary を生成
            case_dict, symptom_dictionary = make_question_and_dictionary(
            ...
            st.session_state.messages.append({
                "role": "assistant",
                "content": f"あなたの発言をもとに問診票をdictionary形式で作成しました。これから構造化データを作成します。:\n{json.dumps(case_dict, ensure_ascii=False)}"
            })
            ...
            case_dict = st.session_state["case_dict"]
            ...
            # step3: 一度要約を提示し、追加コメントをもらう
            # ---------------------------------------------------
            case_dict = st.session_state["case_dict"]
            summary_ver1 = make_summary(case_dict)

            st.session_state["patients_additional_comment"] = user_input

            # 要約をアシスタントメッセージとして追加
            assistant_text = (
                "以下のようにまとめました。ご確認ください。\n\n"
                f"【要約】\n{summary_ver1}\n\n"
                "気になる点があれば入力してください。なければ「特になし」とお書きください。"
            )
            st.session_state["messages"].append({
                "role": "assistant",
                "content": assistant_text,
            ...
            st.session_state["patients_summary_ver1"] = summary_ver1
